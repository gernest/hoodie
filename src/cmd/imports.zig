...
pub fn render(allocator: *mem.Allocator, stream: var, tree: *ast.Tree) anyerror!bool {
...
fn renderRoot(
    ...
    const symbols = &try outline.outlineDecls(allocator, tree);
    defer symbols.deinit();
    outline.Declaration.sortList(symbols);
    ...
    var it = &outline.Declaration.Iterator.init(symbols);
    while (true) {
        var decl = it.next() orelse return;
        // look for zig fmt: off comment
        var start_token_index = decl.node.firstToken();
            ...
            if (mem.eql(u8, mem.trim(u8, tree.tokenSlicePtr(start_token)[2..], " "), "zig fmt: off")) {
                ...
                while (true) {
                    ...
                    if (mem.eql(u8, mem.trim(u8, tree.tokenSlicePtr(end_token)[2..], " "), "zig fmt: on")) {
                        ...
                        while (tree.tokens.at(decl.node.firstToken()).start < end_token.end) {
        ...
        try renderTopLevelDecl(allocator, stream, tree, 0, &start_col, decl.node);
        if (it.peek()) |next_decl| {
            if (next_decl.typ != decl.typ) {
                try stream.writeByte('\n');
                start_col = 0;
            } else {
